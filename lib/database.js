...
class Database {
  ...
  ensureIndexes (callback) {
    ...
    this._sql.serialize(() => {
      this._sql.run('CREATE TABLE IF NOT EXISTS pings (timestamp BIGINT NOT NULL, ip TINYTEXT, playerCount MEDIUMINT, pingMs INTEGER)', handleError)
      ...
      // Migrate existing databases that don't have the pingMs column yet
      this._sql.run('ALTER TABLE pings ADD COLUMN pingMs INTEGER', err => {
  ...
  loadGraphPoints (graphDuration, callback) {
    ...
    this.getRecentPings(startTime, endTime, pingData => {
      ...
      for (const row of pingData) {
        ...
        graphData[2].push(row.pingMs ?? null)
      }
      ...
      Object.keys(relativeGraphData).forEach(ip => {
        ...
        for (const serverRegistration of this._app.serverRegistrations) {
          if (serverRegistration.data.ip === ip) {
            ...
            serverRegistration.loadGraphPoints(startTime, graphData[0], graphData[1], graphData[2])
      ...
      this.getRecord(serverRegistration.data.ip, (hasRecord, playerCount, timestamp) => {
        if (hasRecord) {
          ...
          this.getRecordLegacy(serverRegistration.data.ip, (hasRecordLegacy, playerCountLegacy, timestampLegacy) => {
            ...
            // Insert server entry to records table (OR IGNORE in case of race condition on restart)
            const statement = this._sql.prepare('INSERT OR IGNORE INTO players_record (timestamp, ip, playerCount) VALUES (?, ?, ?)')
  ...
  insertPing (ip, timestamp, unsafePlayerCount, pingMs) {
    this._insertPingTo(ip, timestamp, unsafePlayerCount, pingMs, this._sql)
    ...
    if (dailyDatabase) {
      this._insertPingTo(ip, timestamp, unsafePlayerCount, pingMs, dailyDatabase)
    }
  }

  _insertPingTo (ip, timestamp, unsafePlayerCount, pingMs, db) {
    const statement = db.prepare('INSERT INTO pings (timestamp, ip, playerCount, pingMs) VALUES (?, ?, ?, ?)')
    statement.run(timestamp, ip, unsafePlayerCount, pingMs ?? null, err => {
      if (err) {

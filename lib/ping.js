    ...
    this.startPingTasks(results => {
      ...
      for (const serverRegistration of this._app.serverRegistrations) {
        ...
        if (config.logToDatabase) {
          ...
          this._app.database.insertPing(serverRegistration.data.ip, timestamp, unsafePlayerCount, result.pingMs)
        }
        ...
        const update = serverRegistration.handlePing(timestamp, result.resp, result.err, result.version, updateHistoryGraph, result.pingMs)
    ...
    for (const serverRegistration of this._app.serverRegistrations) {
      ...
      ping(serverRegistration, config.rates.connectTimeout, (err, resp) => {
        ...
        results[serverRegistration.serverId] = {
          ...
          err,
          version,
          pingMs: err ? null : (Date.now() - pingStart)
        }

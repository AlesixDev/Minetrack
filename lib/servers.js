...
class ServerRegistration {
  ...
  handlePing (timestamp, resp, err, version, updateHistoryGraph, pingMs) {
    ...
    if (updateHistoryGraph) {
      ...
      TimeTracker.pushAndShift(this.pingMsGraphData, pingMs ?? null, TimeTracker.getMaxGraphDataLength())
    }

    // Delegate out update payload generation
    return this.getUpdate(timestamp, resp, err, version, pingMs)
  }

  getUpdate (timestamp, resp, err, version, pingMs) {
    const update = {}
  ...
  loadGraphPoints (startTime, timestamps, points, pingMsPoints) {
    this.graphData = TimeTracker.everyN(timestamps, startTime, GRAPH_UPDATE_TIME_GAP, (i) => points[i])
    this.pingMsGraphData = pingMsPoints
      ? TimeTracker.everyN(timestamps, startTime, GRAPH_UPDATE_TIME_GAP, (i) => pingMsPoints[i])
      : []
  }
